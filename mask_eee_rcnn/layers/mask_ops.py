...
def _do_paste_mask(masks, boxes, img_h, img_w, skip_empty=True, mode='bilinear'):
    """
    Args:
        masks: N, 1, H, W
        boxes: N, 4
    ...
    img_masks = F.grid_sample(masks.to(dtype=torch.float32), grid, align_corners=False, mode=mode)
...
def paste_masks_in_image(masks, boxes, image_shape, threshold=0.5, mode='bilinear'):
    """
    ...
    for inds in chunks:
        masks_chunk, spatial_inds = _do_paste_mask(
            masks[inds, None, :, :], boxes[inds], img_h, img_w, skip_empty=device.type == "cpu", mode=mode
        )
