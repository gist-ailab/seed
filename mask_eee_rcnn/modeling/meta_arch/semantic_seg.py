...
from mask_eee_rcnn.layers import Conv2d, ShapeSpec
...
class SemSegFPNHead(nn.Module):
    ...
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        self.sem_seg_sigmoid       = cfg.MODEL.SEM_SEG_HEAD.SEM_SEG_SIGMOID
        # fmt: on
        ...
        for k in range(4):
            norm_module = nn.GroupNorm(32, conv_dims) if norm == "GN" else None
            conv = Conv2d(
                feature_channels["p2"] if k == 0 else conv_dims,
                conv_dims,
                kernel_size=3,
                stride=1,
                padding=1,
            ...
            self.add_module("semantic_conv_{}".format(k+1), conv)
        self.predictor = Conv2d(conv_dims, num_classes, kernel_size=1, stride=1, padding=0)
        weight_init.c2_msra_fill(self.predictor)
    ...
    def forward(self, features, targets=None):
        ...
        x = F.interpolate(x, scale_factor=self.common_stride, mode="bilinear", align_corners=False)
        x_pred = F.interpolate(x_pred, scale_factor=self.common_stride, mode="bilinear", align_corners=False)        
