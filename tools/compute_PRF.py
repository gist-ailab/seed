...
# Codes from https://github.com/NVlabs/UnseenObjectClustering/blob/master/lib/utils/evaluation.py
...
# Code adapted from: https://github.com/davisvideochallenge/davis-2017/blob/master/python/lib/davis/measures/f_boundary.py
def boundary_overlap(i, j, predicted_mask, gt_mask, bound_th=0.003):
    """
    ...
    # cv2.imwrite("tmp_bo_{}_{}_0_pred.png".format(i, j), predicted_mask*255)
    # cv2.imwrite("tmp_bo_{}_{}_0_gt.png".format(i, j), gt_mask*255)
    
    bound_pix = bound_th if bound_th >= 1 else \
            np.ceil(bound_th*np.linalg.norm(predicted_mask.shape))
    # print("{}-{} >>> {} / {} / {}".format(
    #         i, j, bound_pix, bound_th, np.linalg.norm(predicted_mask.shape)))

    # Get the pixel boundaries of both masks
    fg_boundary = seg2bmap(predicted_mask);
    gt_boundary = seg2bmap(gt_mask);
    # cv2.imwrite("tmp_bo_{}_{}_1_pred.png".format(i, j), fg_boundary*255)
    # cv2.imwrite("tmp_bo_{}_{}_1_gt.png".format(i, j), gt_boundary*255)

    # print("{}-{} >>> {} / {} ".format(
    #         i, j, fg_boundary.shape, gt_boundary.shape))
    ...
    # cv2.imwrite("tmp_bo_{}_{}_2_pred.png".format(i, j), fg_dil*255)
    # cv2.imwrite("tmp_bo_{}_{}_2_gt.png".format(i, j), gt_dil*255)
    ...
    # cv2.imwrite("tmp_bo_{}_{}_3_pred.png".format(i, j), fg_match*255)
    # cv2.imwrite("tmp_bo_{}_{}_3_gt.png".format(i, j), gt_match*255)
...
def multilabel_metrics(prediction, gt, obj_detect_threshold=0.75, return_assign=False):
    ...
    for i, gt_i in enumerate(labels_gt):
        ...
        for j, pred_j in enumerate(labels_pred):
            ...
            # mIOU
            B = np.logical_or(pred_j_mask, gt_i_mask)
            iou = np.count_nonzero(A) / np.count_nonzero(B)

            ### Boundary Stuff ###
            boundary_stuff[i,j] = boundary_overlap(i, j, pred_j_mask, gt_i_mask)
    ...
    results = {'Objects F-measure' : F_measure,
    ...
    if return_assign:
        ...
        for i, j in assignments:
            assign_new.append([labels_gt[i], labels_pred[j]])
        return (results, assign_new)
    else:
        return results

def boundary_overlap_occ(i, j, predicted_mask, gt_mask, bound_th=0.003):
    assert np.atleast_3d(predicted_mask).shape[2] == 1
...
def multilabel_amodal_metrics(prediction, gt, obj_detect_threshold=0.75, return_assign=False):
    ...
    for i, gt_i in enumerate(labels_gt):
        ...
        gt_idx = np.where(gt == gt_i)[0][0]
        gt_i_mask = gt[gt_idx] == gt_i

        for j, pred_j in enumerate(labels_pred):
            
            pred_j_mask = prediction[j] == pred_j
    ...
    # for pred_j in labels_pred:
    for j, pred_j in enumerate(labels_pred):
        pred_mask = prediction[j] == pred_j
        boundary_prec_denom += np.sum(seg2bmap(pred_mask))
    boundary_rec_denom = 0. # recall_tps + recall_fns

    for i, gt_i in enumerate(labels_gt):
        gt_idx = np.where(gt == gt_i)[0][0]
        gt_mask = gt[gt_idx] == gt_i
        boundary_rec_denom += np.sum(seg2bmap(gt_mask))
